<template>
  <div class="wgrid clearfix">
    ...
    <div class="long-bet betting">
      <div class="desktop-layout">
        ...
        <div class="balance-wrapper">
          <p class="balance"><span class="text-nowrap"><span class="symbol symbol-coins">{{coin_symbol|coinName}}</span></span>
          </p>
          <p class="new_balance">{{wallet_coin}}</p>
        </div>
        <input type="number" name="amount" placeholder="Minimum bet is 10" autocomplete="off"
               :class="input_class"
               v-model="bet_amount"
               :disabled="change_bet">
      </div>
    ...
    <div class="playfield">
      <div class="bet-row">
        <div class="bombs">
          <div class="bomb" :class="{active: level_hazard===1}" @click="switchLevel(1)">1</div>
          <div class="bomb" :class="{active: level_hazard===3}" @click="switchLevel(3)">3</div>
          <div class="bomb" :class="{active: level_hazard===5}" @click="switchLevel(5)">5</div>
          <div class="bomb" :class="{active: level_hazard===24}" @click="switchLevel(24)">24</div>
        </div>
...
<script>
  ...
  export default {
    components : {},
    mixins : [sndbase, sndmine, StringFilter],
    name : "field-bet",
    computed : {
      bankRolls : {
        get () {
