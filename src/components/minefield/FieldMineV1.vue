  ...
  export default {
    ...
    methods : {
      ...
      genWaves (stage) {
        ...
        let count = 0, movement = 0.51, scale_init = 50.11;
      ...
      genOcean (stage) {
        ...
        let count = 0, movement = 0.21, scale_init = 0.11;
        that.$on ("engineUpdate", ({ dt, df }) => {
          count += 0.0005;
      ...
      genDice (cid, meta, x, y, /*,mine_hit*/) {
        ...
        let state = 0, count = magic_num / 2, savedx = area.position.x, scx = 0.4;
      ...
      showMonsters (list) {
        if (this.showInAnimation) {
          //with animations
          let sh = 0, setloop = 0, that = this, ismonster = false,
            start_wait = 3000, iterate_wait = 600;
          that.playlock = true;

          let iter = [];

          for (let k = 0; k < 25; k++) {
            if (k !== that.requested_draw_loc && this.pickhistory.indexOf (k) === -1) {
              iter.push (k)
            }
          }
          ...
          setTimeout (function () {
            setloop = setInterval (function () {
              if (sh < iter.length) {
                ...
                ismonster = list.indexOf (check_i) > -1;
                if (ismonster) {
      ...
      confirmSafe () {
        ...
        if (this.pickhistory.indexOf (this.requested_draw_loc) === -1) {
      ...
      OfflineStart (level, h, v) {
        ...
        generateMap (h, v, (cid, name, x, y) => {
          const isMonster = orders.indexOf (cid) > -1;
