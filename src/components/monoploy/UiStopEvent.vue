...
<script>
  ...
  export default {
    ...
    methods : {
      ...
      popDialogNewCard (cardEvent) {
        this.data_payload = cardEvent;
        //(cardEvent.pathTile);
        this.statement = "Collected a Card";
        this.title = "Bomb!";
        this.display_window = true;
        if (cardEvent.outstanding_events[0]) {
        }
        this.CountDown (5);
        cardEvent.player.emitter.emit ("collected_item", cardEvent.pathTile, cardEvent.outstanding_events[0]);
      },
      CountDown (sec) {
        if (this.cooldown === 0) {
          this.cooldown = 100;
...
<style scoped lang="scss">
  ...
  .modal-container {
    ...
    #dialog_content {
      ...
      overflow: hidden;
      position: relative;
      //box-shadow: 0px 0px 50px rgba($colorUp, .9);
      @include back_spot_light(8);
      .boximg {
        position: relative;
        display: flex;
        align-items: center;
