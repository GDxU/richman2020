<template>
  <div class="flexcontainx grid control_planel_view">
    <div class="flexhitem flex-banner control_face_cover">
      ...
      <div v-else><span style="vertical-align: middle">{{status_text}}</span></div>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    mounted () {
      EventBus.$on ("login_result", this.login_response);
      const that = this
      that.$nextTick (() => {
        if (window) {
