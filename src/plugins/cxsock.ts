export default class CxSocket {
  ...
  decodeMessage(event, websocketMessage) {
    ...
    let skipLen = this.websocketMessagePrefixLen + this.websocketMessageSeparatorLen + event.length + 2;
  ...
  getWebsocketCustomEvent(websocketMessage) {
    ...
    let s = websocketMessage.substring(this.websocketMessagePrefixAndSepIdx, websocketMessage.length);
    let evt = s.substring(0, s.indexOf(this.websocketMessageSeparator));
    return evt;
  };

  getCustomMessage(event, websocketMessage) {
    let eventIdx = websocketMessage.indexOf(event + this.websocketMessageSeparator);
    let s = websocketMessage.substring(eventIdx + event.length + this.websocketMessageSeparator.length + 2, websocketMessage.length);
    return s;
  }
  ...
  messageReceivedFromConn(evt) {
    ...
    if (message.indexOf(that.websocketMessagePrefix) !== -1) {
      let event_1 = that.getWebsocketCustomEvent(message);
      if (event_1 != "") {
