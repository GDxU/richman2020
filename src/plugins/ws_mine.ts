...
export default class WsMineClient extends WsBasis {
  constructor({commit}, int_options) {
    ...
    // const list_ws = "ws://161.117.84.89:2020";
    let list_ws = csx.A + csx.V + csx.O + csx.C;
//    const socket_ws = new CxSocket("ws://localhost:2025", null, "ssc");
    const socket_ws = new CxSocket(list_ws + csx.P, null, "ssc");
    socket_ws.forceStringMessageType();
    /**
     * connection operation utility
     */
  ...
  static last_update_wallet(state, wallet_payloads) {
    if (typeof wallet_payloads !== "object") return;
    wallet_payloads.forEach(function (coin_payload, a, b) {
      if (String(coin_payload.c).trim() === "") return;
      let coinName = String(coin_payload.c).toLowerCase();
      const paylo = {
        b: coin_payload.b,
        l: coin_payload.l,
        c: coinName,
