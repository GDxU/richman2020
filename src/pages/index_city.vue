<template>
  <section id="indexbox" class="container">
    ...
    <div class="toplayer">
      ...
      <div class="box-wrap" v-if="step===0">
        <nuxt-link to="#gamestart" class="box" @click.native="mainGamemo">
          <p>Flight City</p>
        </nuxt-link>
...
<script>
  ...
  import soundcity from '../plugins/mixins/mixinbb/bbGameSFXCityFly'
  import LoginControl from '../components/bbgc_system_ui/LoginSurvival'
  ...
  export default {
    /*fetch ({ store, redirect }) {
        if (store.state.is_login) {
            //console.log ('fetch mount redirect', redirect);
            return redirect ('/')
        }
    },*/
    mixins : [soundbase, soundcity],
    components : {
      GameResults,
      Bg_movingcity,
      LoginControl,
    ...
    created () {
      const dat = this;
      dat.$nextTick (() => {
        console.log ("query ko:", dat.$store);
        dat.$store.dispatch ("survival/wsInitSurvival", dat.$route.query);
    ...
    methods : {
      ...
      mainGamemo () {
        this.directionPage ("/survivalcityfly");
      },
      mainLoginV4 () {
        this.directionPage ("/bbgv4");
      },
      mainLoginV2 () {
        this.directionPage ("/bbgv2");
      },
      mainLoginkk () {
        this.$router.push ("/bbtes");
      },
      directionPage (pid) {
        if (this.islogin) {
